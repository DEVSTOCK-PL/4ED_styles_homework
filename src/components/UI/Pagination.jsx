    ...
    <div id="paginationContainer" className="w-2/6 flex justify-center items-center sticky top-0">
      <nav>
        <ul className="inline-flex  text-base h-10 divide-x divide-gray-700  py-6">
          <li key={"prev"}>
            <NavLink to={"/rickandmorty/" + (currentPage - 1).toString()} className={styles.prevBtn()}>
              Previous
            </NavLink>
          </li>
          {paginationArray.map((page) => (
            <li key={page}>
              <NavLink to={"/rickandmorty/" + page.toString()} className={styles.btn(currentPage === page)}>
                {page}
              </NavLink>
            </li>
          ))}
          ...
          <li key={"next"}>
            <NavLink to={"/rickandmorty/" + (currentPage + 1).toString()} className={styles.nextBtn()}>
              Next
            </NavLink>
          </li>
        </ul>
